...
#  This work was conducted using SLURM job scheduler.
#  Adapt the scripts to our local configuration.
...
python \
  -W ignore \
  -i \
  /home_lab_local/s2410093/VGAMT/VGAMT/main.py \
  --beam_size 3 \
  --exp_name ${EXP_NAME} \
  --dump_path ${DUMP_PATH} \
  --data_path ${DATA_PATH} \
  ...
  --dropout 0.4 \
  --batch_size 2 \
  --optimizer 'adam_inverse_sqrt,lr=0.0001,warmup_updates=2000' \
  --epoch_size ${EPOCH_SIZE} \
  --eval_bleu true \
  --max_epoch 500 \
  --max_len 80 \
  --num_workers 1 \
  --stopping_criterion "valid_${SRC_LANG}-${TGT_LANG}_mt_bleu,50" \
