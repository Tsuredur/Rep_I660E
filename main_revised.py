...
This main.py is used to solve the dependency conflict problem
and distributed training problem of the original version of main.py.

If you are running on linux or have only one GPU, we recommend using this revised version of main.py.
"""
...
def main(params):
    params.dump_path = params.dump_path.replace('\\', '/')
    # Debug
    if params.debug:
    ...
    #     params.world_size = 1
    if params.local_rank != -1:
        params.global_rank = int(os.environ.get("RANK", 0))  # default RANK = 0
        torch.cuda.set_device(params.local_rank)
        params.device = torch.device(f"cuda:{params.local_rank}" if torch.cuda.is_available() else "cpu")
        dist.init_process_group(backend='nccl', init_method='env://')
        params.world_size = torch.distributed.get_world_size()
    else:
        params.global_rank = 0  # 1 GPU  init rank = 0
        params.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        params.world_size = 1
    ...
    if params.world_size > 1:
        ...
        valid_sampler, test_sampler = None, None  # we do not use distributed training so set it as NONE
    ...
    if params.local_rank != -1:
        ...
        # when simple GPU we use normal model instand of DDP
